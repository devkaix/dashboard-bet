...
async function fetchPlayers(pvrs: PVR[], agents: Agent[], range?: DateRange): Promise<Player[]> {
  ...
  return Array.from(agg.entries()).map(([pid, p]) => {
    ...
    return {
      ...
      kyc_status: meta?.kyc_status || null,
      balance: meta?.balance != null ? Number(meta.balance) : null,
      withdrawable_balance: meta?.withdrawable_balance != null ? Number(meta.withdrawable_balance) : null,
      health_score: null,
